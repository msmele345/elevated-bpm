...
- [x] Every lesson's goal auto-detects completion reliably (no false positives/negatives across the arc) — `detectLessonCompletion` (`src/model/arc.ts`) is the pure transition seam consumed by `App`: it evaluates the active lesson's declarative assertions, emits completion once, and stays quiet for an already-earned lesson after dismissal, re-entry, or hydration. The shipped-arc contract (`src/lessons/lessons.test.ts`) synthesizes a known-good public `GoalContext` for every one of the 14 JSON lessons and then breaks every assertion one at a time; all known-good contexts complete and every partial placement, missing accent/note, undersized count/range/sweep/chord, and incomplete capstone stays incomplete. JSON authoring now fails before runtime for empty or repeated step lists, an unreachable pitch-variety count for a lane, and a knob id the deck does not have (`src/model/lesson.ts`). Covered by Vitest at the evaluator, transition, observation, and shipped-arc seams; the earlier AC1 browser pass exercised all 14 through the real controls in order
- [x] Completing the final lesson delivers a distinct "you made techno" payoff moment — a newly earned data-defined capstone raises `FinaleMoment` (`src/components/FinaleMoment.tsx`): a full-viewport EB-01 graduation plate with a 16-step color run, oversized "YOU MADE TECHNO" hit, and one hardware-style "Keep it rolling" exit over the still-visible deck. While open it is an accessible modal (`role="dialog"`, labelled/described, close control focused), the deck is `inert`/hidden from assistive tech, Escape closes, mapped live-stab keys cannot leak through, Enter/Space still activate the native button, and reduced-motion users get the complete static composition. The payoff is session-only and transition-driven: dismissing it returns to the completed lesson, revisiting the capstone or reloading an already-complete project does not replay it. Verified in-browser at 1280×720 and 390×844: the final edit opened the dialog with all 16 lights and the close control focused; the deck was inert; dismissal restored it; reload kept the lesson earned without reopening the payoff; no Vite overlay or console errors. Covered by Vitest (`arc.test.ts`, `finale.test.ts`, `FinaleMoment.test.ts`)
...
**Note (AC4):** the capstone still completes the moment the user arrives with the whole groove assembled — the lesson correctly recognizes the result rather than forcing one more arbitrary edit. The payoff is tied to that incomplete → complete transition, not to mounting the final lesson, so persisted completion never turns every reload into a modal. Because arc navigation is deliberately ungated, its copy says "Final challenge complete" rather than claiming the entire curriculum was completed out of order.
...
- [x] Cohesive hardware-inspired visual language across drum machine, bass, keyboard, and mixer (consistent spacing, materials, lighting/glow) — the deck's materials are now pressed from one shared token set in `:root` (`src/index.css`): every raised control (steps, transport play, arc stops, lesson dismiss, share/mute/solo buttons) uses the same `--key-face`/`--key-raised`/`--key-pressed` steel, every recessed window (BPM readout, fader track, share URL field, keyboard bed, knob tracks, scope) the same `--well-bg`/`--well-shadow`, every utility plate (arc, lesson panel, resume chip) the same `--strip-face`/`--strip-shadow`, with one `--go` green for every run/earned LED and one `--focus` ring color; ring states (accents, current arc stop) compose on top of `--key-raised` instead of hand-copying it. Spacing became one vertical rhythm: the deck is a flex column with a single `--deck-gap`, replacing the per-strip margins that had drifted (14 vs 18 px). Structure unified too: the drum machine now carries the same `panel-title` name/model plate as bass (BL-303) and stab (CS-08) — "Rhythm Section · DR-909" — and the deck-global transport moved out of the drum panel into a Master strip ("Main Out · MX-01") that also houses the scope and is where AC3's macro knobs will land. Verified in-browser at 1280×900 and 390×844: all four surfaces read as one machine, lesson spotlighting/completion and the playhead sweep still work (four-on-the-floor auto-completed from real clicks mid-playback), zero console errors; all 253 Vitest tests pass
- [x] Live waveform or spectrum visualizer runs on canvas at 60fps without affecting audio — a 16-bar spectrum scope in the Master strip: one bar per step wearing the step grid's quad colors, drawn as LED segment ladders on canvas (`src/components/SpectrumScope.tsx`). The pure math is TDD'd in `src/model/scope.ts` (13 tests): log-spaced bin grouping over 35 Hz–14 kHz so the kick gets as much room as the hats, dB→0..1 normalization in a fixed −78/−8 dB window (silence rests at exactly 0), and instant-attack/limited-fall decay so bars drop smoothly. The engine tap is a `Tone.Analyser('fft', 1024)` connected off the destination as a dead-end branch (`src/audio/engine.ts`) — it only pulls samples when read, never sits in the signal path, so it cannot affect audio. The draw loop runs on rAF reading `engine.getSpectrum()` directly — never React state — and skips the repaint entirely on frames where no segment changes; reduced-motion users get one static unlit ladder with no loop at all. Verified in-browser while playing: one full draw (levels + 224 segment rects) measured **0.017 ms** against the 16.7 ms frame budget, and the frame cadence with the scope visible was identical to the scope hidden — the test window's own throttled ~40 Hz vsync was the ceiling (a blank iframe measured the same), with the idle page at the display's full rate; transport stayed `started` throughout with zero console errors, and canvas pixels were confirmed changing frame to frame
- [x] Master macro knobs (filter, drive) audibly shape the full mix and persist in `ProjectState` — two `Knob`s on the MX-01 Master strip beside the scope, driving a real master bus: every voice (drum gains, bass filter, stab pool) now feeds one shared input, then the whole mix runs filter → drive → destination (`createMasterBus`, `src/audio/engine.ts`). The filter is a -12 dB/oct lowpass — the DJ-mixer master filter, gentler than the bass voice's -24 dB sound-design filter — and drive is a `Tone.Distortion` whose wet mix *and* curve both ride the knob (`masterBusParams`, `src/model/master.ts`, TDD'd), so 0% is genuinely dry (wet 0, bit-transparent) and 100% is fully wet at a 0.9 curve; cutoff and wet ramp over 20 ms and the distortion curve is only rebuilt when the amount changes, so dragging can't zipper or churn. Both macros rest neutral (filter 18 kHz, drive 0%) — an untouched deck sounds exactly as before the bus. Persistence: `instrumentSettings.master` rides a v7 `ProjectState` (v6→v7 migration defaults the macros via `createMasterSettings`, which clamps/repairs like the bass patch), and the share payload validates master ranges alongside bass. Verified in-browser while playing: closing the filter from the real knob (Home key) dropped the 3–10 kHz band from −54 dB to −88 dB while the kick's 40–300 Hz band lost only 5 dB; full drive raised sustained low-band energy ~10 dB with new high harmonics at unchanged peak level; 4 kHz / 40% survived autosave → reload → back onto the live nodes (filter.frequency 4003.5 Hz, wet 0.4, distortion 0.36) after the first gesture, and the previous session's v6 document migrated in place keeping its kick and earned lesson; transport stayed `started` through every knob move, zero console errors. Covered by Vitest: `master.test.ts` (specs, clamping, repair, bus mapping, % readout), `projectState.test.ts` (v7 document, `setMasterParamValue`, v6→v7), `share.test.ts` (round-trip with a driven master, out-of-range/missing macros rejected) — 266 tests passing, `tsc -b` and `vite build` clean
